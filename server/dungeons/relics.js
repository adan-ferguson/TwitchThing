...
const VALUE_MULTIPLIER = 1.27
const valueScaler = new ScaledValue(VALUE_MULTIPLIER)
...
function minorRelic(adventurerInstance, dungeonRun){
  return {
    rewards: {
      xp: Math.floor(10 * calcPower(dungeonRun.floor) * (0.5 + Math.random() / 2))
    },
...
function majorRelic(adventurerInstance, dungeonRun){
  return {
    rewards: {
      xp: Math.floor(50 * calcPower(dungeonRun.floor) * (0.5 + Math.random() / 2))
    },
...
function calcPower(floor){
  return valueScaler.getVal(floor - 1)
}